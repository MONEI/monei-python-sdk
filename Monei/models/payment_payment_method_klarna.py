...
class PaymentPaymentMethodKlarna(BaseModel):
    """
    Details from Klarna order used as payment method at the time of the transaction. 
    """ # noqa: E501
    billing_category: Optional[StrictStr] = Field(default=None, alias="billingCategory")
    auth_payment_method: Optional[StrictStr] = Field(default=None, alias="authPaymentMethod")
    __properties: ClassVar[List[str]] = ["billingCategory", "authPaymentMethod"]

    @field_validator('billing_category')
    def billing_category_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['PAY_LATER', 'PAY_NOW', 'SLICE_IT', 'SLICE_IT_BY_CARD']):
            raise ValueError("must be one of enum values ('PAY_LATER', 'PAY_NOW', 'SLICE_IT', 'SLICE_IT_BY_CARD')")
        return value

    @field_validator('auth_payment_method')
    def auth_payment_method_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['invoice', 'fixed_amount', 'pix', 'base_account', 'deferred_interest', 'direct_debit', 'direct_bank_transfer', 'b2b_invoice', 'card', 'slice_it_by_card']):
            raise ValueError("must be one of enum values ('invoice', 'fixed_amount', 'pix', 'base_account', 'deferred_interest', 'direct_debit', 'direct_bank_transfer', 'b2b_invoice', 'card', 'slice_it_by_card')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PaymentPaymentMethodKlarna from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PaymentPaymentMethodKlarna from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "billingCategory": obj.get("billingCategory"),
            "authPaymentMethod": obj.get("authPaymentMethod")
        })
        return _obj
