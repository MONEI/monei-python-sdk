...
class PaymentSequenceRecurring(BaseModel):
    """
    Specific configurations for recurring payments. Will only be used when `sequence`.`type` is `recurring`. 
    """ # noqa: E501
    expiry: Optional[StrictStr] = Field(default='*(The payment method or card expiration)*', description="Date after which no further recurring payments will be performed. Must be formatted as `YYYYMMDD`. ")
    frequency: Optional[StrictInt] = Field(default=25, description="The minimum number of **days** between the different recurring payments. ")
    __properties: ClassVar[List[str]] = ["expiry", "frequency"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PaymentSequenceRecurring from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PaymentSequenceRecurring from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "expiry": obj.get("expiry") if obj.get("expiry") is not None else '*(The payment method or card expiration)*',
            "frequency": obj.get("frequency") if obj.get("frequency") is not None else 25
        })
        return _obj
