...
class PaymentMessageChannel(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """PaymentMessageChannel - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] (str): The channel used to send the payment message. if `customerEmail` or `customerPhone` is provided this parameter is ignored. ., must be one of ["EMAIL", "SMS", ]  # noqa: E501

        Keyword Args:
            value (str): The channel used to send the payment message. if `customerEmail` or `customerPhone` is provided this parameter is ignored. ., must be one of ["EMAIL", "SMS", ]  # noqa: E501
